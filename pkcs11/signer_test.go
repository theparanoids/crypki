...
func testCollector(t *testing.T, ctx context.Context, reqChan chan interface{}) {
	for {
		req := <-reqChan
		switch r := req.(type) {
		case Request:
			signer, err := r.Pool.Get(ctx)
			if err != nil {
				r.RespChan <- nil
			}
			r.RespChan <- signer
...
func TestSignX509RSACert(t *testing.T) {
	...
	testcases := map[string]struct {
		...
		"cert-bad-signer":        {ctx, certRSA, defaultIdentifier, proto.Priority_Unspecified_priority, true, true},
		"cert-request-cancelled": {cancelCtx, certRSA, defaultIdentifier, proto.Priority_High, false, true},
	}
	for label, tt := range testcases {
		label, tt := label, tt
		t.Run(label, func(t *testing.T) {
			signer := initMockSigner(crypki.RSA, caPriv, caCert, tt.isBadSigner)
			go testCollector(t, ctx, reqChan)
			if tt.ctx == cancelCtx {
				cancel()
			}
			data, err := signer.SignX509Cert(tt.ctx, tt.cert, tt.identifier, tt.priority, reqChan)
			if err != nil != tt.expectError {
...
func TestSignX509ECCert(t *testing.T) {
	...
	testcases := map[string]struct {
		...
		"cert-ec-good-signer": {ctx, certEC, defaultIdentifier, proto.Priority_High, false, false},
		"cert-ec-bad-signer":  {ctx, certEC, badIdentifier, proto.Priority_Medium, false, true},
	}
	for label, tt := range testcases {
		label, tt := label, tt
		t.Run(label, func(t *testing.T) {
			signer := initMockSigner(crypki.ECDSA, caPriv, caCert, tt.isBadSigner)
			go testCollector(t, tt.ctx, reqChan)
			data, err := signer.SignX509Cert(tt.ctx, tt.cert, tt.identifier, tt.priority, reqChan)
			if err != nil != tt.expectError {
				t.Fatalf("%s: got err: %v, expect err: %v", label, err, tt.expectError)
			}
